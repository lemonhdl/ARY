...
func InitTokenEncoders() {
	...
	if err != nil {
		logger.SysLog(fmt.Sprintf("WARNING: failed to download gpt-3.5-turbo token encoder (%s), token counting will use approximation", err.Error()))
	} else {
		defaultTokenEncoder = gpt35TokenEncoder
	}
	gpt4oTokenEncoder, err := tiktoken.EncodingForModel("gpt-4o")
	if err != nil {
		logger.SysLog(fmt.Sprintf("WARNING: failed to download gpt-4o token encoder (%s)", err.Error()))
	}
	gpt4TokenEncoder, err := tiktoken.EncodingForModel("gpt-4")
	if err != nil {
		logger.SysLog(fmt.Sprintf("WARNING: failed to download gpt-4 token encoder (%s)", err.Error()))
	}
	...
	// Fallback: use approximation if no encoder downloaded
	if defaultTokenEncoder == nil {
		logger.SysLog("no token encoder available, enabling approximate token counting")
