...
async function pollProxyStatus() {
  try {
    var res = await fetch("http://localhost:3738/__dcr/status", { cache: "no-cache" });
    if (!res.ok) return updateProxyUI(false);
    var data = await res.json();
    if (!data.running) return updateProxyUI(false);

    $("#proxyState").textContent = "🟢 运行中";
    $("#proxyState").style.color = "var(--green)";
    $("#proxyUpstream").textContent = data.upstream || "—";
    $("#proxyChainId").textContent = (data.chainId || "").slice(0, 12) + "…";
    $("#proxyEntries").textContent = (data.totalEntries || 0) + " 条";
    if (data.lastRequest) {
      $("#proxyTokens").textContent = (data.lastRequest.tokens || 0).toLocaleString() + " 🪙";
      $("#proxyFiles").textContent = (data.lastRequest.fileCount || 0) + " 文件";
    }
  } catch(e) {
    updateProxyUI(false);
  }
...
function updateProxyUI(running) {
  if (running) return;
  $("#proxyState").textContent = "⚫ 未启动";
  $("#proxyState").style.color = "var(--muted)";
}
...
(async function () {
  ...
  setInterval(pollProxyStatus, 3000);
})();
