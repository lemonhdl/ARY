...
- `client-source/` 与 `proxy-source/` 是 A 组交付的源码回放原型，默认使用 `3737/3738/3000`。
- 已安装的 `DCR Desktop App` 桌面壳在本机实测暴露的是统一本地服务 `http://127.0.0.1:4302`，健康检查入口为 `/api/source/v1/health`。

排查桌面壳是否正常启动时，应优先检查 `4302`；只有在直接运行本目录源码回放原型时，才使用 `3737/3738` 作为判据。
...
| 面板 | 地址 | 用途 |
|------|------|------|
| DevCompass 本地服务 | `http://127.0.0.1:4302` | 桌面壳统一入口，返回会话列表、统计与 DCR runtime API |
